...
function setup() {
  ...
  push();
  // TODO can x and y be fully parameterized based on xres and yres
  translate((3.5 / xres) * width, (1.5 / yres) * height);
  rectMode(CENTER);
  imageMode(CENTER);
  drawFrontPage();
  pop();
  ...
  // drawing page 1
  push();
  translate((3.5 / xres) * width, (0.5 / yres) * height);
  rotate(Math.PI);
  rectMode(CENTER);
  imageMode(CENTER);
  drawInnerPage(1);
  ...
  // drawing page 2
  push();
  translate((2.5 / xres) * width, (0.5 / yres) * height);
  rotate(Math.PI);
  rectMode(CENTER);
  imageMode(CENTER);
  drawInnerPage(2);
  ...
  // drawing page 3
  push();
  translate((1.5 / xres) * width, (0.5 / yres) * height);
  rotate(Math.PI);
  rectMode(CENTER);
  imageMode(CENTER);
  drawInnerPage(3);
  ...
  // drawing page 4
  push();
  translate((0.5 / xres) * width, (0.5 / yres) * height);
  rotate(Math.PI);
  rectMode(CENTER);
  imageMode(CENTER);
  drawInnerPage(4);
  ...
  // drawing page 5
  push();
  translate((0.5 / xres) * width, (1.5 / yres) * height);
  rectMode(CENTER);
  imageMode(CENTER);
  drawInnerPage(5);
  pop();
  ...
  // drawing page 6
  push();
  translate((1.5 / xres) * width, (1.5 / yres) * height);
  rectMode(CENTER);
  imageMode(CENTER);
  drawInnerPage(6);
  pop();
  ...
  push();
  translate((2.5 / xres) * width, (1.5 / yres) * height);
  rectMode(CENTER);
  imageMode(CENTER);
  drawBackPage();
  pop();
...
function drawFrontPage() {
  ...
  let g = createGraphics(width, height, WEBGL);
  g.noStroke();
  g.pixelDensity(1)
  g.background("white");
...
function drawInnerPage() {
  ...
  let g = createGraphics(width, height, WEBGL); // Set up WebGL context
  ...
  // textAlign()
  // textSize(40);
  let text1 = random(dataArray).content;
  ...
  let tSize = map(text1.length, mintextlength, maxtextlength, 84, 36);
  ...
  wrapTexture(textCanvas, dverts1, subdivisionRows, subdivisionCols, g);
  image(g, 0, 0, pageWidth, pageHeight)
...
function drawBackPage() {
  ...
  let g = createGraphics(pageWidth, pageHeight, WEBGL);
  g.noStroke();
  g.pixelDensity(1);

  g.background(random(palette));
  ...
  distortion = random(200, 300);
  distortWidth = textCanvas1.width;
  distortHeight = textCanvas1.height;
  let dverts1 = getDistortedVertices(distortion, distortWidth, distortHeight, subdivisionRows, subdivisionCols);
  wrapTexture(textCanvas, dverts1, subdivisionRows, subdivisionCols, textCanvas1);
  ...
  let dverts2 = getDistortedVertices(distortion, distortWidth, distortHeight, subdivisionRows, subdivisionCols);
  wrapTexture(textCanvas2, dverts2, subdivisionRows, subdivisionCols, textCanvas3);
...
function keyPressed() {
  ...
  if (key === 's') {
    saveCanvas('high-res-image', 'png');  // Save the image as PNG
  }
}