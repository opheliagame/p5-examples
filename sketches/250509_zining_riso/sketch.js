// https://christopherswift.work/Chapter-3-Zines-and-care
...
// https://editor.p5js.org/caleececconi/sketches/W3DcVTp8Y
// https://rednoise.org/rita/index.html#examples
// https://rednoise.org/rita/index.html#reference
// Fonts - Mingzat, Playpen Sans

let img;
let cols = 10; // Number of columns in the grid
let rows = 10; // Number of rows in the grid
let w = 400; // Width of the rectangle
let h = 400; // Height of the rectangle
let distortion = 320; // Magnitude of the distortion
let vertices = []; // Array to store distorted vertices

function preload() {
  // img = loadImage("mushi.png"); // Load the texture image
  // img = loadImage('jousenji1.png'); // Load the texture image
}

function setup() {
  createCanvas(800, 800); // Set up WebGL context
  noStroke(); // Disable stroke for cleaner results
  ...
  for (let y = 0; y <= rows; y++) {
    ...
    for (let x = 0; x <= cols; x++) {
      ...
      let xPos = map(x, 0, cols, -w / 2, w / 2);
      let yPos = map(y, 0, rows, -h / 2, h / 2);
      ...
      let dx = ((noise(x * 0.1, y * 0.1) - 0.5) * distortion) / 2; // Centered and scaled noise for x distortion
      let dy = ((noise(x * 0.1 + 100, y * 0.1) - 0.5) * distortion) / 2; // Centered and scaled noise for y distortion

      // Store the distorted vertex position
      vertices[y][x] = createVector(xPos + dx, yPos + dy);
    }
  }
...
function draw12() {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      let v0 = vertices[y][x]; // Top-left corner
      let v1 = vertices[y][x + 1]; // Top-right corner
      let v2 = vertices[y + 1][x + 1]; // Bottom-right corner
      let v3 = vertices[y + 1][x]; // Bottom-left corner
      ...
      let u0 = map(x, 0, cols, 0, 1),
        v0_uv = map(y, 0, rows, 0, 1); // Top-left UV
      let u1 = map(x + 1, 0, cols, 0, 1),
        v1_uv = map(y, 0, rows, 0, 1); // Top-right UV
      let u2 = map(x + 1, 0, cols, 0, 1),
        v2_uv = map(y + 1, 0, rows, 0, 1); // Bottom-right UV
      let u3 = map(x, 0, cols, 0, 1),
        v3_uv = map(y + 1, 0, rows, 0, 1); // Bottom-left UV

      // Draw the first triangle (v0, v1, v2)
      vertex(v0.x, v0.y, 0, u0, v0_uv); // Top-left corner
      vertex(v1.x, v1.y, 0, u1, v1_uv); // Top-right corner
      vertex(v2.x, v2.y, 0, u2, v2_uv); // Bottom-right corner

      // Draw the second triangle (v0, v2, v3)
      vertex(v0.x, v0.y, 0, u0, v0_uv); // Top-left corner
      vertex(v2.x, v2.y, 0, u2, v2_uv); // Bottom-right corner
      vertex(v3.x, v3.y, 0, u3, v3_uv); // Bottom-left corner
    }
  }
...
function spaceOut(text1, g) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    can.textSize(random(22, 32));
    let w = can.textWidth(word + " ".repeat(space));

    if (remw < w) {
      can.translate(-(width - remw), 32);
      remw = width;
    }

    let isNoun = RiTa.isNoun(word);
    let c = isNoun ? "blue" : "black";
    can.fill(c);
...
let data = [
  ...
  {
    slug: "families-without children",
    title: "Families without children",
    content:
  ...
  {
    slug: "hetero-gay-family",
    title: "Hetero-gay family",
    content:
  ...
  {
    slug: "non-marital-child rearing",
    title: "Non-marital child rearing",
    content:
  ...
  {
    slug: "polygamous-family",
    title: "Polygamous family",
    content:
  ...
  {
    slug: "you-can create your own family",
    title: "You can create your own family",
    content:
